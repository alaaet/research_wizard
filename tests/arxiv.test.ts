import { ArxivRetriever } from '../backend/search_client/retrievers/arXiv';

async function testArxivRetriever() {
  const retriever = await ArxivRetriever.create();
  const topics = [
  ...
  for (const topic of topics) {
    ...
    console.log(`\nQuery: ${topic}`);
    console.log('arXiv Results:', results);
  }
}
