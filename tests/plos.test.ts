import { PlosRetriever } from '../backend/search_client/retrievers/plos';

async function testPlosRetriever() {
  const retriever = await PlosRetriever.create();
  const topics = [
  ...
  for (const topic of topics) {
    ...
    console.log(`\nQuery: ${topic}`);
    console.log('PLOS Results:', results);
  }
}
